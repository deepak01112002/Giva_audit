...
class FormContainer extends Component {
	...
	async getFormDataApiCall() {
		...
		if (userData) {
			...
			if (this.props.formID !== undefined && this.props.formID != '' && userData['user_type'] != Role.user) {
	...
	getQuestinaire(data) {
		...
		for (let a in data) {
			...
			if (rawMarks == 'yes' || rawMarks == 'no' || rawMarks == 'N/A' || rawMarks == 'NA') {
				let marks = rawMarks == 'yes' ? 1 : 0;
				answerData['marks'] = marks;
				answerData['max_marks'] = 1;
			}
			if (!data[a].non_scoring) {
				if (data[a]['question'].includes('0 to 5') || data[a]['question'].includes('1 to 5')) {
		...
		const sortedArray = questionaire.sort((a, b) => a.question_no - b.question_no);
		return sortedArray;
	}
			...
			if (user) {
				...
				await Promise.all(
					this.state.images.map(async (val, i) => {
						...
						ourData[val.formId][val.name]['answer'] = imageResponse.payload.data.data.result.file.filename;
						return;
					})
				);
				let submitFormData = JSON.parse(JSON.stringify(this.props.form_data.answerContent));
				...
				if(this.state.seleceStoreIndex){
					...
					let cIndex = allCampaign.findIndex((item) => item.name == audit_detail.campaign_name.answer);
					...
					let index = allStores.findIndex((item) => item.store_name == audit_detail.store_name.answer);
	...
	render() {
		...
		return (
			<>
						...
						<Container>
							{activeFormId ? (
								<Form
									...
									isNextValid={this.state.activeFormIndex < this.props.form_data.tabs.length - 1}
								/>
							) : null}
						</Container>
					</>
				...
				<BackdropProgress
					open={this.state.formDataSubmitting || fetchSubmitDataLoading || defaultFormDataLoading}
				/>
				<Snackbar
