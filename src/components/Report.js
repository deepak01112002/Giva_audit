import { Page, View, Document, StyleSheet, Text, Image, Font } from '@react-pdf/renderer';
...
const AudiTextetails = (props) => {
	...
	return (
		...
		>
			<Text style={{ ...styles.questionnarieBox, padding: clientConfiguration[client_code].auditDetailsPadding }}>{item?.key}:</Text>
			<Text style={{ ...styles.questionnarieBox, flex: 4, padding: clientConfiguration[client_code].auditDetailsPadding }}>
				{item?.value}
			</Text>
		</View>
	);
			...
			<Page>
				<View style={styles.container}>
					...
					{clientConfiguration[client_code].logoSource && (
						<Image style={styles.logo} src={clientConfiguration[client_code].logoSource} />
					)}
				</View>
				<View
					style={{
				...
				>
					...
					{data.audit_details && data.audit_details.length > 0
						? data?.audit_details.map((item, i) => <AudiTextetails item={item} key={i} index={i} />)
				...
				{clientConfiguration[client_code].overAllExperience && (
					...
					>
						...
						<View style={{ display: 'flex', width: '100%', flexDirection: 'row' }}>
							...
							<View style={{ width: '40%', display: 'flex', padding: '5px 0px' }}>
								<View>
									{scoreCriteria.map((item, i) => (
										...
										>
											...
											<Text style={styles.questionnarieBox}>{item.description}</Text>
										</View>
									))}
								</View>
							</View>
								...
								<View
									style={{
										...
										borderColor: getAttributeByScore(
											(categoryResult.marks_scored / categoryResult.category_marks) * 100
										),
									}}
								>
									<View
									...
									>
										<Text style={{ fontWeight: 'bold', marginRight: '5px', fontSize: '10px' }}>
											Total marks
										</Text>
										<Text style={{ fontWeight: 'bold', marginRight: '5px', fontSize: '10px' }}>
